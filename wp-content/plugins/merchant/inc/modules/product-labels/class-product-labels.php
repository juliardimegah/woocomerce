...
class Merchant_Product_Labels extends Merchant_Add_Module {
	...
	public function get_labels( $product, $context = 'both' ) {
        ...
        $multiple_labels = isset( $settings['multiple_labels'] ) ? $settings['multiple_labels'] : false;
		...
		if ( isset( $settings['labels'] ) ) {
			...
			foreach ( $labels as $label ) {
				...
				$is_excluded = $this->is_product_excluded( $product->get_id(), $label );
				if ( $is_excluded ) {
					continue;
				}
				...
				switch ( $display_rule ) {
					case 'featured_products':
						$should_display = $this->is_featured( $product );
						break;

					case 'products_on_sale':
						$should_display = $this->is_on_sale( $product );
						break;

                    case 'by_category':
                    case 'by_tags':
                    case 'by_brands':
                        // Map display rules to their corresponding taxonomy and slug array
                        $taxonomy_map = array(
                                'by_category' => array(
                                        'taxonomy' => 'product_cat',
                                        'slugs'    => $label['product_cats'] ?? array(),
                                ),
                                'by_tags'     => array(
                                        'taxonomy' => 'product_tag',
                                        'slugs'    => $label['product_tags'] ?? array(),
                                ),
                                'by_brands'   => array(
                                        'taxonomy' => 'product_brand',
                                        'slugs'    => $label['product_brands'] ?? array(),
                                ),
                        );

                        $taxonomy = $taxonomy_map[ $display_rule ]['taxonomy'];
                        $slugs    = $taxonomy_map[ $display_rule ]['slugs'];
                    ...
                    case 'out_of_stock':
						$should_display = $this->is_out_of_stock( $product );
						break;

					case 'pre-order':
						$should_display = $this->is_pre_order( $product );
						break;

					case 'new_products':
						if ( isset( $label['new_products_days'] ) ) {
							$should_display = $this->is_new( $product, $label['new_products_days'] );
						}
						break;
					...
					case 'specific_products':
						...
						$should_display = in_array( $product->get_id(), $product_ids, true );
						break;
				...
				if ( $should_display ) {
					// Process sale data for this label
					...
					// Replace {shortcodes} by real values for this label
					$label_html = str_replace(
						array(
							'{sale}',
							'{sale_amount}',
							'{inventory}',
					...
					// If multiple labels is disabled, return after first match (backward compatibility)
					if ( ! $multiple_labels ) {
						break;
					}
				}
			...
			// If no matching labels, return empty
			if ( empty( $matching_labels ) ) {
				return '';
			}

			// If single label mode or only one label, use old structure for backward compatibility
			if ( ! $multiple_labels || count( $matching_labels ) === 1 ) {
				$label_data = $matching_labels[0];
				...
				return '<div class="' . esc_attr( $classes ) . '" style="' . merchant_array_to_css( $styles ) . '">' . $label_html . '</div>';
			}

			// Multiple labels mode - position each label individually
			$output = '';
			$device_visibility_classes = '';
			...
			foreach ( $matching_labels as $label_data ) {
				...
				$label_shape_class = '';
				if ( $label_type === 'text' ) {
					$label_shape_class = ' merchant-product-labels__' . ( $label['label_text_shape'] ?? 'text-shape-1' );
				}

				// Get device visibility classes from first label (for main container)
				if ( empty( $device_visibility_classes ) && isset( $label['show_devices'] ) ) {
					foreach ( $label['show_devices'] as $device ) {
						$device_visibility_classes .= ' show-on-' . $device;
					}
				}

				// Get all styles for this label (including positioning)
				$label_styles = $this->get_shape_based_styles( $label );

				$label_classes = 'merchant-product-labels__label-wrapper';
				$label_classes .= ' merchant-product-labels__' . $label_type;
				$label_classes .= $label_shape_class;
				$label_classes .= $label['position_anchor'] === 'top-right' ? ' position-top-right' : ' position-top-left';

				$output .= '<div class="' . esc_attr( $label_classes ) . '" style="' . merchant_array_to_css( $label_styles ) . '">' . $label_html . '</div>';
			}

			// Wrap all labels in main container
			$main_classes = 'merchant-product-labels';
			$main_classes .= ' merchant-product-labels__' . ( ( $is_shortcode && $context === 'single' ) ? 'shortcode' : 'regular' );
			$main_classes .= ' merchant-product-labels--multiple';
			$main_classes .= $device_visibility_classes;

			return '<div class="' . esc_attr( $main_classes ) . '">' . $output . '</div>';
		} else {
			// legacy mode support.
